...
public class DefaultNpmHostedRepository
        ...
        try {
            ...
            if (packageRequest.isMetadata()) {
              ...
              if (packageRequest.isRegistryRoot()) {
                contentLocator = hostedMetadataService.produceRegistryRoot();
              } else if (packageRequest.isPackageRoot()) {
                contentLocator = hostedMetadataService.producePackageRoot(packageRequest.getName());
              } else {
                contentLocator = hostedMetadataService.producePackageVersion(packageRequest.getName(), packageRequest.getVersion());
              }
    ...
    public void storeItem(ResourceStoreRequest request, InputStream is, Map<String, String> userAttributes)
        ...
        try {
            ...
            try {
                final PackageRoot packageRoot = hostedMetadataService.consumePackageRoot(new PreparedContentLocator(is, NpmRepository.JSON_MIME_TYPE, ContentLocator.UNKNOWN_LENGTH));
